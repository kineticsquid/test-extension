...
var require_scheduler = __commonJS({
  "../node_modules/.pnpm/observable-fns@0.6.1/node_modules/observable-fns/dist/_scheduler.js"(exports) {
    ...
    var AsyncSerialScheduler2 = class {
    ...
    exports.AsyncSerialScheduler = AsyncSerialScheduler2;
  }
});
...
function distinctUntilChanged(isEqualFn = import_isEqual.default) {
  return (observable) => {
    return new observable_default((observer) => {
      ...
      const scheduler = new import_scheduler.AsyncSerialScheduler(observer);
      const subscription = observable.subscribe({
        complete() {
          scheduler.complete();
        },
...
var NeedsAuthChallengeError = class extends AuthError {
  constructor() {
    super(
...
function isNeedsAuthChallengeError(error) {
...
// ../lib/shared/src/sourcegraph-api/utils.ts
...
async function addAuthHeaders(auth, headers, url) {
...
`;
var EVALUATE_FEATURE_FLAGS_QUERY = `
...
var DURABLE_REFETCH_INTERVAL_HINT = {
  initialInterval: 60 * 60 * 1e3,
  // 1 hour
  backoff: 1
};
var TRANSIENT_REFETCH_INTERVAL_HINT = {
...
var SourcegraphGraphQLAPIClient = class _SourcegraphGraphQLAPIClient {
  ...
  static withGlobalConfig() {
    return new _SourcegraphGraphQLAPIClient(resolvedConfig.pipe(distinctUntilChanged()));
  }
  /**
  ...
  async isValidSiteVersion({ minimumVersion, insider = true }, signal) {
    ...
    return import_semver.default.gte(version, minimumVersion);
  }
  async contextSearch({
    repoIDs,
    query,
  ...
  async queryPrompts({
    ...
    const gqlQuery = hasOrderByRelevance ? PROMPTS_QUERY : hasIncludeViewerDraftsArg ? LEGACY_PROMPTS_QUERY_6_3 : LEGACY_PROMPTS_QUERY_5_8;
    const input = {
      query,
      first: first ?? 100,
      recommendedOnly,
      ...
      builtinOnly,
      orderBy: hasOrderByRelevance ? "PROMPT_RELEVANCE" /* PROMPT_RELEVANCE */ : void 0,
      orderByMultiple: hasOrderByRelevance ? void 0 : orderByMultiple || [
   ...
   */
  async getEvaluatedFeatureFlags(flagNames, signal) {
    ...
    if (hasEvaluateFeatureFlags) {
      ...
      return this.fetchSourcegraphAPI(
        ...
        return extractDataOrError(
          response,
          (data) => data.evaluateFeatureFlags?.reduce(
            (acc, { name, value }) => {
              acc[name] = value;
              return acc;
            },
...
function setJSONAcceptContentTypeHeaders(headers) {
...
function isCustomAuthChallengeResponse(response) {
  function isIncomingMessageType(v) {
    return "httpVersion" in response;
  }
  const statusCode = isIncomingMessageType(response) ? response.statusCode : response.status;
  if (statusCode === 401) {
    const headerEntries = isIncomingMessageType(response) ? Object.entries(response.headers) : Array.from(response.headers.entries());
...
var GraphQLTelemetryExporter = class {
  constructor(allowedDevEvents) {
    this.allowedDevEvents = allowedDevEvents;
  }
  isEventAllowed(event) {
    if (this.allowedDevEvents === void 0) {
      return true;
    }
...
function getTier(authStatus2) {
  return !authStatus2.authenticated ? 3 /* NotAuthenticated */ : 2 /* Enterprise */;
}
...
var TelemetryRecorderProvider = class extends import_telemetry.TelemetryRecorderProvider {
  constructor(config) {
    ...
    super(
      ...
      process.env.CODY_TELEMETRY_EXPORTER === "testing" ? TESTING_TELEMETRY_EXPORTER.withAnonymousUserID(config.clientState.anonymousUserID) : new GraphQLTelemetryExporter(config.allowedDevEvents),
      [
